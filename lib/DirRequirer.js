  ...
  (function(modules, factory) {
    ...
    return DirRequirer = (function() {
                ...
                return (function(dependency) {
                  ...
                  if (callback != null) {
                    return requirejs([dependency], function(content) {
                      return callback(content, path.relative(requirejs.toUrl(''), dependency.slice(0, -3)));
                    });
                  } else {
                    return promises.push(When.promise(function(resolve) {
                      return requirejs([dependency], function(content) {
                        return resolve({
                          filename: path.relative(requirejs.toUrl(''), dependency.slice(0, -3)),
                          content: content
                        });
                      });
                    }));
      ...
      DirRequirer.prototype.defineAll = function(pathname, additionalDeps, callback) {
        ...
        if (files != null) {
          dirDeps = (function() {
            ...
            for (_i = 0, _len = files.length; _i < _len; _i++) {
              filename = files[_i];
              if (path.extname(filename) === this.options.extension) {
                _results.push(path.join(pathname, path.basename(filename, this.options.extension)));
              }
            }
            return _results;
          }).call(this);
        ...
        return requirejs.define(deps, function() {
